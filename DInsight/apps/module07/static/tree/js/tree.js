 function AccordionMenu(options,classname) {
	this.config = {
		containerCls        : classname,                // 外层容器
		menuArrs            :  '',                         //  JSON传进来的数据
 ...
 AccordionMenu.prototype = {
	...
	_renderHTML: function(container){
		...
		$(_config.menuArrs).each(function(index,item){
			var lihtml = $('<li><a class="mulu"><img src="/static/tree/images/blank.gif" alt=""/>'+item.name+'</a></li>');
	...
	_createSubMenu: function(submenu,lihtml){
		...
		$(submenu).each(function(index,item){
			...
			subLi = $('<li><a class="itemli" id="'+item.id+'" href="'+url+'">'+item.name+'</a></li>');
			if(item.submenu && item.submenu.length > 0) {
				subLi = $('<li><a class="itemli" href="'+url+'">'+item.name+'</a></li>');
				$(subLi).children('a').prepend('<img src="/static/tree/images/blank.gif" alt=""/>');
	...
	_levelIndent: function(ulList){
		...
		var initTextIndent = 3,
			lev = 1,
			$oUl = $(ulList);
		
		while($oUl.find('ul').length > 0){
			initTextIndent = parseInt(initTextIndent,10) + 1 + 'em'; 
			$oUl.children().children('ul').addClass('lev_' + lev)
